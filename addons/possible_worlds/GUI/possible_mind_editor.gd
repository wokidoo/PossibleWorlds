...
var _tension_row:TreeItem
var _diff_row:TreeItem
var root:TreeItem
...
func _update_tree():
	...
	_tension_row = create_item(get_root())
	_tension_row.set_cell_mode(0,TreeItem.CELL_MODE_STRING)
	_tension_row.set_editable(0,true)
	_tension_row.set_cell_mode(1,TreeItem.CELL_MODE_CUSTOM)
	_tension_row.set_text(1,"Create Proposition...")
	_tension_row.set_cell_mode(2,TreeItem.CELL_MODE_STRING)
	_tension_row.set_text_alignment(3,HORIZONTAL_ALIGNMENT_RIGHT)
	_tension_row.set_text(3,"Internal Tension: ")
	_tension_row.set_cell_mode(3,TreeItem.CELL_MODE_STRING)
	_tension_row.set_text(4,str(possible_world_mind.get_total_internal_tension()))
	_tension_row.set_selectable(2,false)
	_tension_row.set_selectable(3,false)
	_tension_row.set_selectable(4,false)
	_diff_row = create_item()
	_diff_row.set_text_alignment(3,HORIZONTAL_ALIGNMENT_RIGHT)
	_diff_row.set_text(3,"Diff: ")
	_diff_row.set_text(4,str(possible_world_mind.get_total_internal_diff()))
...
func _on_cell_selected():
	if get_selected() == _tension_row and get_selected_column() == 1:
		_create_new_prop()
	elif get_selected().get_meta('prop') != null and get_selected_column() == 4:
...
func _create_new_prop():
	var prop :StringName = _tension_row.get_text(0)
